...
fn tab_width(ws: &crate::workspace::Workspace, tab_idx: usize, show_index: bool) -> u16 {
    display_width_u16(&tab_chrome_label(ws, tab_idx, show_index))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}
...
fn layout_tab_hit_areas(
    ...
    for (idx, rect) in rects.iter_mut().enumerate().skip(scroll) {
        ...
        let desired = tab_width(ws, idx, show_index);
...
fn centered_tab_scroll(ws: &crate::workspace::Workspace, area: Rect, show_index: bool) -> usize {
    ...
    for scroll in 0..=ws.active_tab {
        let rects = layout_tab_hit_areas(ws, area, scroll, show_index);
...
fn max_tab_scroll(ws: &crate::workspace::Workspace, area: Rect, show_index: bool) -> usize {
    (0..ws.tabs.len())
        .find(|&scroll| {
            layout_tab_hit_areas(ws, area, scroll, show_index)
                .last()
...
pub(crate) fn compute_tab_bar_view(
    ...
    if !mouse_chrome {
        let max_scroll = max_tab_scroll(ws, area, show_index);
        let scroll = if follow_active {
            centered_tab_scroll(ws, area, show_index).min(max_scroll)
        } else {
            current_scroll.min(max_scroll)
        };
        return TabBarView {
            scroll,
            tab_hit_areas: layout_tab_hit_areas(ws, area, scroll, show_index),
    ...
    let all_tabs = layout_tab_hit_areas(ws, all_tabs_area, 0, show_index);
    ...
    let max_scroll = max_tab_scroll(ws, tab_area, show_index);
    let scroll = if follow_active {
        centered_tab_scroll(ws, tab_area, show_index).min(max_scroll)
    } else {
        current_scroll.min(max_scroll)
    };
    let tab_hit_areas = layout_tab_hit_areas(ws, tab_area, scroll, show_index);
