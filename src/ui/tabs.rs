...
fn tab_width(ws: &crate::workspace::Workspace, tab_idx: usize, show_index: bool, sep: &str) -> u16 {
    display_width_u16(&tab_chrome_label(ws, tab_idx, show_index, sep))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}
...
fn tab_chrome_label(
    ...
    let display = if show_index && has_custom_name {
        ...
        format!("{}{sep} {name}", tab_idx + 1)
    } else if show_index {
        format!("{}{sep}", tab_idx + 1)
    } else {
...
fn layout_tab_hit_areas(
    ...
    for (idx, rect) in rects.iter_mut().enumerate().skip(scroll) {
        ...
        let desired = tab_width(ws, idx, show_index, sep);
...
fn centered_tab_scroll(
    ...
    for scroll in 0..=ws.active_tab {
        let rects = layout_tab_hit_areas(ws, area, scroll, show_index, sep);
...
fn max_tab_scroll(
    ...
    (0..ws.tabs.len())
        .find(|&scroll| {
            layout_tab_hit_areas(ws, area, scroll, show_index, sep)
                .last()
...
pub(crate) fn compute_tab_bar_view(
    ...
    if !mouse_chrome {
        let max_scroll = max_tab_scroll(ws, area, show_index, sep);
        let scroll = if follow_active {
            centered_tab_scroll(ws, area, show_index, sep).min(max_scroll)
        } else {
            current_scroll.min(max_scroll)
        };
        return TabBarView {
            scroll,
            tab_hit_areas: layout_tab_hit_areas(ws, area, scroll, show_index, sep),
    ...
    let all_tabs = layout_tab_hit_areas(ws, all_tabs_area, 0, show_index, sep);
    ...
    let max_scroll = max_tab_scroll(ws, tab_area, show_index, sep);
    let scroll = if follow_active {
        centered_tab_scroll(ws, tab_area, show_index, sep).min(max_scroll)
    } else {
        current_scroll.min(max_scroll)
    };
    let tab_hit_areas = layout_tab_hit_areas(ws, tab_area, scroll, show_index, sep);
...
pub(super) fn render_tab_bar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    for (idx, tab) in ws.tabs.iter().enumerate() {
        ...
        let name = tab_chrome_label(ws, idx, app.tab_index_numbers, app.index_number_separator());
...
mod tests {
    ...
    fn zoom_marker_counts_toward_tab_width() {
        ...
        assert_eq!(tab_width(&ws, 0, true, ")"), 18);
    }
